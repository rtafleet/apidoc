# Get access tokens to call RTA Graph
...
RTA Graph exposes a rich set of granular permissions over the resources it controls. These permissions are expressed as strings and grant apps access to RTA Graph resources like vehicles, parts, work orders, purchase orders, etc. For example:

* _Vehicle.Read_ allows an app to read the vehicles in the RTA system.
* _Part.Update_ allows an app to update part details.

There are two types of permissions:

* Delegated permissions are used by apps that run with a user present. The user's privileges are delegated to the app which makes calls on behalf of the user to RTA Graph. Many of these permissions can be consented to by a user, but others require administrator consent.
...
For a complete list of RTA Graph permissions, as well as the differences between delegated and application permissions, see the [Permissions](#permissions) reference.
...
Some apps call RTA Graph with their own identity and not on behalf of a user. In many cases, these are background services or daemons that run on a server without the presence of a signed-in user. An example of such an app might be an data extraction service that wakes up and runs overnight. In some cases, apps that have a signed-in user present may also need to call RTA Graph under their own identity. For example, an app may need to use functionality that requires more elevated privileges in an organization than those carried by the signed-in user.

Apps that call RTA Graph with their own identity use the OAuth 2.0 client credentials grant flow to get access tokens from Auth0. In this topic, we will walk through the basic steps to configure a service and use the OAuth client credentials grant flow to get an access token.

### Authentication and authorization steps

The basic steps required to configure a service and get a token from the Azure AD v2.0 endpoint that your service can use to call RTA Graph under its own identity are:

1. Register your app.
1. Configure permissions for RTA Graph on your app.
...
* `access_token`: The requested access token. Your app can use this token in calls to RTA Graph.
...
Apps that call RTA Graph under their own identity fall into one of two categories:

* Background services (daemons) that run on a server without a signed-in user.
* Apps that have a signed-in user but also call RTA Graph with their own identity; for example, to use functionality that requires more elevated privileges than those of the user.
* Apps that call RTA Graph with their own identity use the OAuth 2.0 client credentials grant to authenticate with Azure AD and get a token.